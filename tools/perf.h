...
public:
    ...
    inline uint64_t merge_count(bool all = true)
    {
        ...
        for (int i = 0; i < thread_num; i++) {
            if (req_retry_count[i] == 0)
                continue;
            if (all)
                printf("%lu ", req_retry_count[i][0] - req_retry_count[i][1]);
        ...
        for (int i = 0; i < thread_num; i++) {
            if (req_count[i] == 0)
                continue;
            if (all)
                printf("%lu ", req_count[i][0] - req_count[i][1]);
    ...
    inline uint64_t merge_count_by_id(int id)
    {
        ...
        for (int i = 0; i < thread_num; i++) {
            if (req_count[i] == 0)
                continue;
            if (id <= 1)
                printf("%lu ", req_count[i][id] - req_count[i][id + 8]);
    ...
    inline uint64_t get_real_time_tps(int thread_id)
    {
        ...
        if (interval > 1000000000ull) {
            ...
            printf("tps = %lu %lu %lu (%lu %lu %lu %lu) %lu \n", tps,
    ...
    void merge_print(std::string filename, int thread_id = 0, int append = 1)
    {
        if (thread_id != 0)
            return;
        ...
        FILE* fp = freopen(("latency" + filename + ".txt").c_str(), append == 1 ? "a" : "w", stdout);
        // printf("total_num = %lu\n", ans[range]);
        // LOG(INFO) << filename << "[latency_cdf]: begin";
        uint64_t l_min = 0;
        for (int i = 0; i < range; i++) {
            if (ans[i]!=0 && l_min == 0){
                l_min = i * time_u;
    ...
    latency_evaluation_t(int _thread_num)
    {
        ...
        for (uint i = 0; i < min(_thread_num, 24); i++) {
            epoch[i] = 0;
        }
        throughput = 0;
        g_mpl = 1;

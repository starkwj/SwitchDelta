...
public:
    ...
    typedef Masstree::tcursor_batch<table_params> cursor_batch_type;
    ...
    void insert(void* key_addr, uint32_t key_size, uint64_t value) {
        Str key = vector_to_Str(key_addr, key_size);
    ...
    void insert(uint64_t int_key, uint64_t value) {
        uint64_t key_buf;
    ...
    void batch_insert(uint64_t* key, uint64_t* value, int cnt) {
        uint64_t key_buf;
        Str keystr = make_key(key[0], key_buf);
        cursor_type lp(table_, keystr);
    ...
    bool search(void* key_addr, uint32_t key_size, uint64_t &value) {
        // uint64_t key_buf;
        Str key = vector_to_Str(key_addr, key_size);
    ...
    bool scan(void* key_addr, uint32_t key_size, std::vector<uint64_t> &vec, int cnt) {
        Str key = vector_to_Str(key_addr, key_size);
    ...
    bool remove(void* key_addr, uint32_t key_size) {
        Str key = vector_to_Str(key_addr, key_size);
        cursor_type lp(table_, key);
        lp.find_locked(*ti);
        lp.finish(-1, *ti);
...
private:
    ...
    static inline Str vector_to_Str(void* key_addr, uint32_t key_size) {
        return Str((char *)key_addr, key_size);
    }
};

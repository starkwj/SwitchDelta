...
public:
    ...
    PROMISE(void) insert(uint64_t int_key, uint64_t value) {
        uint64_t key_buf;
        Str key = make_key(int_key, key_buf);
        cursor_type lp(table_, key);
        // if (found) {
        AWAIT lp.find_insert(*ti);
    ...
    PROMISE(bool) search(uint64_t int_key, table_params::value_type &value) {
        uint64_t key_buf;
        Str key = make_key(int_key, key_buf);
        bool found = AWAIT table_.get(key, value, *ti);
        // printf("%d %lx %lx %lx\n", found, int_key, key_buf,value);
        RETURN found;
    }

    void scan(uint64_t int_key, int cnt, std::vector<table_params::value_type> &vec) {
        uint64_t key_buf;
        Str key = make_key(int_key, key_buf);
    ...
    PROMISE(bool) remove(uint64_t int_key) {
        uint64_t key_buf;
        Str key = make_key(int_key, key_buf);
        cursor_type lp(table_, key);
        AWAIT lp.find_locked(*ti);
        lp.finish(-1, *ti);
        RETURN true;
    }
    /*

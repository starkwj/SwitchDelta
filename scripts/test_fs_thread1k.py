...
def run():
    ...
    for test in itertools.product(DN_THREADS, MN_THREADS, GFLAGS, tmp_coros):
        kill_all(nodes, exe)
        comm = f'--block_size=1024 --qp_type=1 {test[2]} --c_thread={COROS[test[3]][0]}  --dn_thread={test[0]} --mn_thread={test[1]} --read=0'
                ...
                for k in GFLAGS:
                    ...
                    for test in itertools.product(i_list, j_list, k_list, tmp_coros):
                        if test in res:
                            ...
                            xx1 = float(res[test][2])
                            xx2 = float(res[test][3])
                            x1_line.append(xx1 / 1024) # read bw
                            x2_line.append(xx2 / 1024) # write bw 
                            # 6:p50 8:p99
                    ...
                    print(*x_line, *y_line, *x1_line, *x2_line)

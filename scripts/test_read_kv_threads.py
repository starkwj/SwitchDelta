...
def run():
    ...
    for test in itertools.product(DN_THREADS, MN_THREADS, GFLAGS, tmp_coros):
        kill_all(nodes, exe)
        comm = f'--qp_type=1 {test[2]} --c_thread={COROS[test[3]][0]}  --dn_thread={test[0]} --mn_thread={test[1]} --read=50'
        args = comm + f' --coro_num={COROS[test[3]][1]}'
        # def run_test_common(NODES, EXE_NAME, ARGS, last_id, PATH):
        if (test[2] == GFLAGS[0]):
            load_switch("./overwriterun.sh")
        ...
        lat, _x = run_a_test("./tmp_cmd.sh", 120, 18)
        ...
        for i, n in enumerate(nodes):
            ...
            with open("tmp_cmd.sh", "w") as file2:
                file2.writelines("set -x\n");
                file2.writelines(cmd);
            tp, _x = run_a_test("./tmp_cmd.sh", 120, 18)
            restp += float(tp[0][3])
        # print(restp)
        res[test] = [restp, lat]

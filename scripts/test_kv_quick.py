...
#line type
KVEXE = ["kv_co --visibility=false --batch=false --batch_size=1", "kv_co --visibility=true --batch=true --batch_size=16"]
        ...
        for i in DN_THREADS:
            ...
            if 1 == 1:
                ...
                for k in KVEXE:
                    ...
                    for test in itertools.product(i_list, MN_THREADS,  xx, k_list):
                        if test in res:
                            if res[test] is None:
                                print("error")
                                print(test)
                            ...
                            wlat = float(res[test][1][1][6][0:-1]) 
                            rlat = float(res[test][1][2][6][0:-1]) 

                            # 6:p50 8:p99
                            
                            y_line.append(yy) # p50 latency
                            x_line.append(xx) # throughput
                            read_lat.append(rlat)
                            write_lat.append(wlat)
                    ...
                    print(*x_line, *y_line, *write_lat, *read_lat)
        
                plt.xlabel("throughput(Mops/s) " + name + " " + os.path.basename(__file__))
                plt.ylabel("latency(us)") 
                plt.legend()
                plt.savefig("./pic/" + exe + f'DN{i}{k}' +".jpg")

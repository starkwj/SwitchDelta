...
register rpc_id_check { // hash -> key
    width : 32; 
    instance_count : 65536; 
}
...
blackbox stateful_alu write_data_reply_hash {
    reg: key_check;
...
blackbox stateful_alu write_index_reply_hash {
    reg: key_check;

    update_lo_1_value: 0;
    update_hi_1_value: kvmsg.key32;

    output_value: alu_hi;
    output_dst: umeta.key32;
}

blackbox stateful_alu check_index_msg_hash {
    reg: key_check;
...
action new_index_hash() { // data reply + index req
    write_data_reply_hash.execute_stateful_alu(kvmsg.key);
}

action delete_index_hash() {
    write_index_reply_hash.execute_stateful_alu(kvmsg.key);
}

action check_index_hash() {
    // register_read(); // how to read only low-32-bit ? 
    check_index_msg_hash.execute_stateful_alu(kvmsg.key);
}

...
DEFINE_string(index, "hash", "index type");
DEFINE_string(order, "order", "order or norder");
DEFINE_uint64(keyspace, 100 * MB, "key space");
DEFINE_uint32(server0_th, 1, "th count of server0");
DEFINE_uint32(server1_th, 1, "th count of server1 ");
DEFINE_int32(thread_cnt, 1, "thread_num");

DEFINE_int32(first, 1, "first thread");
DEFINE_int32(second, 1, "second thread");


// const int kFirstThread = 1;
// const int kBgThread = 2;

const int kMaxQPSize = 64;
...
struct SimpleCtx {
    ...
    uint8_t server_id_bg;
    uint8_t thread_id_bg;
...
public:
	...
	void batch_process() {
...

		{
			for (uint i = 0; i < batch_cnt; i++) {
				if (test_cnt % 1000 == 0) {
                	if (test_cnt != 0)
						reporter->many_end(1000, thread_id,0,0);
					reporter->begin(thread_id,0,0);
            	}
				test_cnt++;
	...
	void client_worker_(CoroMaster &sink, uint32_t coro_id) {
		...
		cworker_new_request_order(sink, coro_id);
	...
	void run_client_CPC() {
		...
		while (true) {
			...
			for (int i = 0; i < coro_num; i++) {
				if (batch_coro_ctx[i].free && bg_cnt < kMaxQPSize) {
					(*batch_coro_slave[i])(); // new request
				}
				if (batch_coro_ctx[i].async_op_cnt == 0 && batch_coro_ctx[i].free == false && bg_cnt < kMaxQPSize) {
					batch_coro_ctx[i].async_op_cnt = -1;
			...
			while ((wc = wc_list->get_next_wc())) {
				...
				if (c_id != -1) {
					if (FLAGS_order == "norder" && ((RawKvMsg*)reply) -> rpc_header.dst_s_id == 1) {
						bg_cnt--;
                	}
					else {
	...
	void cworker_new_request_order(CoroMaster& sink, uint32_t coro_id) {
		auto& app_ctx = batch_simple_ctx[coro_id];
		auto& ctx = batch_coro_ctx[coro_id];
		reporter->begin(thread_id, coro_id, 0);

		app_ctx.server_id = 0;
		app_ctx.thread_id = mehcached_zipf_next(&op_state) % FLAGS_first;
        app_ctx.server_id_bg = 1;
        app_ctx.thread_id_bg = mehcached_zipf_next(&op_state) % FLAGS_second;
		app_ctx.wait = true;
        ...
        qp = rpc_get_qp(rpc::FLAGS_qp_type, app_ctx.server_id_bg,
			app_ctx.thread_id_bg, qp_id);
		msg = (RawKvMsg*)qp->get_send_msg_addr();

        msg->rpc_header = {
			.dst_s_id = app_ctx.server_id_bg,
			.src_s_id = server_id,
			.src_t_id = thread_id,
			.fake_src_c_id = (uint8_t)coro_id,
			.local_id = 0,
        ...
        if (FLAGS_order == "norder") {
            qp->post_appended_smsg();
			bg_cnt++;
			app_ctx.wait = false;
            // batch_coro_ctx[coro_id].sub_op_cnt();
        }
        else
            qp->post_appended_smsg(&sink);

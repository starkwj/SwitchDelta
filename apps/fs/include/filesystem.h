...
#include "storage.h"                  /* Storage class, definition of node hash and and hash functions. */
#include "debug.h"                    /* Debug class. */
...
public:
    ...
    bool extentRead(const char *path, uint64_t size, uint64_t offset, file_extent_info *fei,  file_pos_info *fpi); /* Allocate read extent. */
    bool extentReadEnd(uint64_t key, char* path);
    bool extentWrite(const char *path, uint64_t size, uint64_t offset, file_pos_info *fpi, file_extent_info *fei_in, file_extent_info *fei_out, uint64_t DataBaseAddress); /* Allocate write extent. Unlock is implemented in updateMeta. */
    bool extentMetaWrite(const char *path, uint64_t size, uint64_t offset, file_extent_info* fei);

    ...
    if (index == NULL) {
        ...
        mutexBitmapItems.lock();        /* Lock table bitmap. */
        {
            if (headFreeBit == NULL) {  /* If there is no free bit in bitmap. */
                Debug::debugItem("table create error");
        ...
        Debug::debugItem("Table::create (block): *index = %lu, &(items[*index]) = %lx\n", *index, &(items[*index]));
    ...
    if (buffer == NULL) {
        ...
        if (count % 8 != 0) {
            ...
            */
            ...
            if (the_last != nullptr) {
                the_last->nextFreeBit = headFreeBit;
            }
        }
    }
}

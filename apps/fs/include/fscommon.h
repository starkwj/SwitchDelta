...
#define kMaxServerCnt 2

typedef int nrfs;
typedef char* nrfsFile;

#define MAX_MESSAGE_BLOCK_COUNT 12      /* Max count of block index in a message. */
...
struct file_extent_info
{
	...
	uint64_t len;
	file_extent_tuple tuple[MAX_MESSAGE_BLOCK_COUNT];
...
/** Definitions. **/
#define MAX_FILE_EXTENT_COUNT 64        /* Max extent count in meta of a file. */
#define BLOCK_SIZE (4 * 1024)    /* Current block size in bytes. */
#define MAX_FILE_NAME_LENGTH 50         /* Max file name length. */
#define MAX_DIRECTORY_COUNT 32         /* Max directory count. */
#define NULLBLCOK_ID ((uint64_t)-1)         /* Max directory count. */
